...
function _init(basepath, mappingFilename) {
	var fs = require('fs');
	...
	if ( !allmodulesexist ) { throw new Error("gimport unable to locate one or more modules. Check config file '" + GIMPORT_MAPPING_JSON_FILENAME + "'."); }
	 ...
	 * Params:
	 *   basepath (optional): base path or root of the solution. Inside that root should exist GIMPORT_MAPPING_JSON_FILENAME file
	 *      with the mappings.
...
gimport.init = function(basepath, mappingFilename) {
	...
	_init(basepath, mappingFilename);
};

/*
 * Same than init() but allowing multiple calls to initialize the module.
 * This is useful for testing when multiple files with tests should initialize
 * the module.
 * Main application entry point should use init() and leave initm() for tests code.
 */
gimport.initm = function(basepath, mappingFilename) {
	_init(basepath, mappingFilename);
}
